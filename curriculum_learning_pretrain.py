...
from crowd_layer.crowd_layers import CrowdsClassification, MaskedMultiCrossEntropy, CrowdsClassificationSModel, \
    ...
    MaskedMultiCrossEntropyCurriculumChannelMatrix, CrowdsClassificationSModelforPretrain
...
pretrained = False
if model == 'B':
    channel_layer = CrowdsClassificationSModelforPretrain(N_CLASSES, N_ANNOT)
    save_weight = True
elif model == 'SW+B':
...
fname_random_weights = 'simple_model_weight.random.hdf5'

if pretrained:
    crowd_model.load_weights(fname_random_weights)
...
if save_weight:
    crowd_model.save_weights(fname_random_weights, overwrite=True)
